...
$Curricula = ''; 
  ...
  if($_SERVER["REQUEST_METHOD"]=="POST"){
       ...
       $curricula = input_check($_POST["curricula"]);
...
if(mysqli_num_rows(mysqli_query($db_link,$sql)) > 0){
   $sql= "UPDATE vcselfstudy_ssf SET `curriculumDesign` ='$curriculum', `updatingCurricula`='$curricula' WHERE `accreditationID`='$accreditationID'";
 if(!mysqli_query($db_link,$sql)){
...
Degree Programmes offered by the University</label>
    <div class="form-group" >
        <textarea class="form-control" id="curriculum" rows="5" cols="60" name="curriculum" required="required"><?php echo $Curriculum?></textarea>
  </div>
  </div>
...
of each programme offered.</label>
    <div class="form-group">
        <textarea class="form-control" id="curricula" required="required" rows="3" cols="58" name="curricula"><?php echo $Curricula?></textarea>
  </div>
  </div>
